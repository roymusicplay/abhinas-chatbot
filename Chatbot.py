...
async def type_and_send(message):
    ...
    if "Luna" in response:
        responsee = response.replace("Luna", "Abhinas")
    else:
        responsee = response
    if "Aco" in responsee:
        responsess = responsee.replace("Aco", "Abhinas")
    else:
        responsess = responsee
    if "Who is Tiana?" in responsess:
        responsess2 = responsess.replace("Who is Abhinas?", "Heroine Of Telegram")
    else:
...
async def main():
    ...
    print(
        """
Your AbhinasChatbot Is Deployed Successfully.
"""
    )
    await idle()
